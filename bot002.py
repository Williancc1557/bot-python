...
from Economia.economia import Criar_conta, Conta, Desc_edit, Transferir, Diaria, Cor, Roubar, Arma, Fenicoins, \
    Top_global, Roubar_banco
from Principais.principais import Avaliar, bot, Avatar, Invite, mydb
from adm.suporte import Add_suporte, Remove_suporte, Identificar, Clean_fenix, Ajuda_suporte, Add_fenicoins, \
    Remove_fenicoins, Set_fenicoins, Ban_fenix, Unban_fenix
...
async def manutenção(ctx):
    await ctx.reply(
        '<:error:788824695184424980>| Esse comando foi **Desativado** temporariamente pela equipe de suporte do fênix!')
    return False
...
async def invite(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        await ctx.channel.send(
            '**<:error:788824695184424980>| {} Você Precisa Esperar `{:.0f} Segundos` Para Dar O Comando Novamente**'.format(
                ctx.author.mention, error.retry_after))
...
async def ajuda_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        await ctx.channel.send(
            '<:error:788824695184424980>| {} Você Precisa Esperar  **Milissegundos** Para Dar O Comando Novamente'.format(
                ctx.author.mention, error.retry_after))
...
async def desafiar_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        await ctx.channel.send(
            '**<:error:788824695184424980>| {} Esse canal precisa esperar `{:.0f} Segundos` Para Dar O Comando `f!desafiar fênix`Novamente**'.format(
                ctx.author.mention, error.retry_after))
...
async def diaria_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        await ctx.channel.send(
            '**<:error:788824695184424980> {} Você precisa esperar `{:.1f} horas` Para Dar O Comando `f!diaria`Novamente**'.format(
                ctx.author.mention, error.retry_after / 3600))
...
async def roubar_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        await ctx.channel.send(
            '**<:error:788824695184424980> {} Você precisa esperar `{:.1f}` **Horas** Para Dar O Comando `f!roubar` Novamente**'.format(
                ctx.author.mention, error.retry_after / 3600))
    if isinstance(error, commands.MemberNotFound):
        await ctx.channel.send(
            f'<:error:788824695184424980> Desculpe {ctx.author.mention}, Procurei o membro `{error.argument}` por todo canto mas acabei não o encontrando!')
...
async def banco_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        await ctx.channel.send(
            '**<:error:788824695184424980>| {} Você precisa esperar `{:.0f} Horas` Para Dar O Comando `f!roubar_banco`Novamente**'.format(
                ctx.author.mention, error.retry_after / 3600))
...
async def apostar(ctx, member: discord.Member = None, dindin: int = None):
    if member == ctx.author:
        await ctx.channel.send(
            '<:error:788824695184424980> Você não pode apostar fenicoins com **si mesmo**, seu bobinho.')
        return
    ...
    if not dindin:
        await ctx.channel.send(
            '<:error:788824695184424980> Você precisa escrever a **quantidade** de *fenicoins* que deseja apostar!!')
        return

    if dindin <= 10:
        await ctx.channel.send(
            '<:error:788824695184424980> Ai não da meu parceiro, tem q apostar **um pouquinho mais de fenicoins** *para eu poder cobrar aquela taxinha de entrada massa de 5%*.')
        return
    ...
    if valores_lidos2[0] < dindin:
        await ctx.channel.send(
            f'<:error:788824695184424980>| Você {ctx.author.mention}, não possui **fenicoins** suficiente para a aposta!')
        return
    ...
    try:
        ...
        if valores_lidos3[0] < dindin:
            await ctx.channel.send(
                f'<:error:788824695184424980>| O(a) {member.mention} não possui **fenicoins** suficiente para a aposta!')
            return
    except:
        pass
    ...
    if member:
        ...
        a = await ctx.channel.send(
            f'Ei {member.mention}, parece que o(a) {ctx.author.mention} quer fazer uma aposta com você! Onde cada um cada um vai pagar **{dindin - (dindin * 0.05)} fenicoin ( Vou cobrar uma taxa de {0.05 * dindin:.0f} fenicoin xD )**\n\n🤝| Para aceitar a aposta, você {member.mention} deve clicar na reação ✅')
...
async def on_reaction_add(reaction, user):
    ...
    if autor == user and react == '<a:emoji_45:815378376528887859>':
        ...
        embed.add_field(name='<:emoji_12:778982435920150538> Mensagem De Bem Vindo',
                        value='`f!welcome` ou `f!bemvindo`\nㅤ', inline=False)
        embed.add_field(name='<a:warn:818918476915540020> Penalizar Alguém (warn)', value='`f!warn <@user> <motivo>`',
                        inline=False)
        embed.add_field(name='<a:warn:818918476915540020> Despenalizar Alguém (warn_remove)',
                        value='`f!warn_remove <@user>`', inline=False)
        embed.add_field(name='<a:warn:818918476915540020> Ver Penalidade (warn)',
                        value='`f!warn_checks <@user>` ou `f!checks_warn <@user>`\nㅤ', inline=False)
        embed.set_thumbnail(
            url='https://media.discordapp.net/attachments/788064370722340885/823220448351354910/20210317_111410.jpg?width=650&height=650')
...
async def on_message_edit(before, after):
    await On_message_edit(after=after, before=before)
